...
public class FuncionApi {
    ...
    @GetMapping("/funciones/{sede}/{peli}")
    public List<GloFuncion> getFunciones(@PathVariable("sede") int sede, @PathVariable("peli") int peli) {
            System.out.println("Get all Categoria Producto...");
            List<GloFuncion> entidad = new ArrayList<>();
            funcionServicio.listarEntidadDato(sede,peli).forEach(entidad::add);
        return entidad;
    }
